function suggestEntityAndRelation(entityData) {
	...
	var toast = notification.createInfo("Empfehlungen verfügbar", "<div class='row' style='margin-left: 0.33333%;'><p>Es gibt Empfehlungen für weitere Beziehungs- und ggf. Entitätstypen ausgehend von " + entityData.entityName + ". Sollen diese angezeigt werden?</p></div><div class='row col-md-offset-1' style=\"margin-left: 0.33333%;\"><div></div><button type='button' class='btn btn-raised btn-success btn-block' style='width: 45%; margin: 2.5%' id='suggestYes'>Yes</button><button type='button' class='btn btn-raised btn-danger btn-block' style='width: 45%; margin: 2.5%' id='suggestNo'>No</button></div>", entityData.entityName);
